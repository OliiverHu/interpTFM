...
This module owns F1 alignment + heldout reporting logic.

Key guarantees (to avoid the "all zeros" failure mode):
...
This is intentionally "library" code used by scripts/* drivers.
"""
...
def _load_scgpt_decoder(scgpt_ckpt: str) -> Callable[[int], str]:
    """
    Build vocab-id -> token string decoder from scGPT tokenizer.
    Uses adapter to avoid importing scgpt internals here.
    """
    ...
    *,
    ...
    for shard in shards:
        ...
        if token_ids is None:
            continue
        # sample some ids for speed (split stability doesn't need all)
    ...
    for shard in tqdm(shards, desc=f"heldout:{layer}"):
        ...
        # process each heldout split separately
        for sp, sp_id in (("valid", 0), ("test", 1)):
    ...
    # selection: threshold -> concept -> [features]
